...
def gate_config() -> dict[str, object]:
    return {
        ...
        "minimum_instruments": 4,
        "minimum_asset_families": 2,
        "minimum_instruments_per_asset_family": 2,
...
def test_promotion_gate_requires_broad_balanced_metric_wins() -> None:
    instruments = [
        make_instrument(),
        make_instrument(),
        make_instrument(joint_wins=3, asset_family="interest_rates"),
        make_instrument(joint_wins=3, asset_family="interest_rates"),
    ]
    ...
    assert result["holdout_joint_wins"] == 4
    assert result["walk_forward_joint_wins"] == 16
    assert result["asset_family_count"] == 2
    assert result["minimum_instruments_per_asset_family"] == 2
...
def test_promotion_gate_requires_independent_asset_family_evidence() -> None:
    instruments = [make_instrument(), make_instrument(), make_instrument(), make_instrument()]
...
def test_promotion_gate_requires_multiple_instruments_per_asset_family() -> None:
    instruments = [
        make_instrument(),
        make_instrument(),
        make_instrument(),
    ...
    depth_check = next(
        requirement
        for requirement in result["requirements"]
        if requirement["name"] == "instrument depth per asset family"
    )
